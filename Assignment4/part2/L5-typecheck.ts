// L5-typecheck
// ========================================================
import { equals, map, zipWith, reduce, zip, KeyValuePair } from 'ramda';
import {
    isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
    isPrimOp, isProcExp, isProgram, isStrExp, isVarRef, parseL5Exp, unparse,
    AppExp, BoolExp, DefineExp, Exp, IfExp, LetrecExp, LetExp, NumExp,
    Parsed, PrimOp, ProcExp, Program, StrExp, isLetValuesExp, LetValuesExp, CExp, VarDecl, BindingValues
} from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import {
    isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
    parseTE, unparseTExp,
    BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isTupleTExp, isNonEmptyTupleTExp, isEmptyTupleTExp, NonTupleTExp, parseTExp, makeNonEmptyTupleTExp, isNonTupleTExp
} from "./TExp";
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> =>
    bind(isValues(app.rator) ? typeofValues(app.rands, tenv) :
        typeofExp(app.rator, tenv), (ratorTE: TExp) => {
...
const isValues = (rator: CExp): boolean => isPrimOp(rator) && rator.op === 'values';

const typeofValues = (rands: CExp[], tenv: TEnv): Result<TExp> => {
    return bind(mapResult((rand: CExp) => typeofExp(rand, tenv), rands),
        (texps: TExp[]) => bind(mapResult(unparseTExp, texps),
            (types: string[]) => bind(parseTE(`(${types.join(" * ")} -> ${types.join(" * ")})`)
                , (parsed: TExp) => isProcTExp(parsed) ? isTupleTExp(parsed.returnTE) ? makeOk(parsed) :
                    isNonTupleTExp(parsed.returnTE) ? makeOk(makeProcTExp(parsed.paramTEs, makeNonEmptyTupleTExp([parsed.returnTE]))) :
                        makeFailure('Fuck you') : makeFailure("No really, just fuck right off"))));
}
...
export const typeOfLetValues = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
    const varArrs = map((b: BindingValues) => map((v: VarDecl) => v.var, b.vars), exp.bindingValuess);
    const varArrsTEs = map((b: BindingValues) => map((v: VarDecl) => v.texp, b.vars), exp.bindingValuess);
    const vals = map((b) => b.valExp, exp.bindingValuess);
    const constraints = zipWithResult((varsTEs: TExp[], val: CExp) => bind(typeofExp(val, tenv),
        (typeOfVal: TExp) => validateVarsVal(varsTEs, typeOfVal, exp)), varArrsTEs, vals)
    return bind(constraints, _ => typeofExps(exp.body, makeExtendTEnv(unload2D(varArrs), unload2D(varArrsTEs), tenv)));
}

const validateVarsVal = (varTEs: TExp[], tuple: TExp, exp: LetValuesExp): Result<true> => {
    return isNonEmptyTupleTExp(tuple) && varTEs.length === tuple.TEs.length ?
        bind(zipWithResult((varTE: TExp, valTE: NonTupleTExp) => checkEqualType(varTE, valTE, exp), varTEs, tuple.TEs), () => makeOk(true)) :
        isEmptyTupleTExp(tuple) && varTEs.length === 0 ? makeOk(true) :
            safe2((varsTEs: string, tupleTEs: string) => makeFailure<true>(`Invalid binding for let values: ${varsTEs}, ${tupleTEs}`))
                (bind(mapResult((unparseTExp), varTEs), (typeArr: string[]) => makeOk(`(${typeArr.join(', ')})`)), unparseTExp(tuple));
}

const unload2D = (arr: any[][]) => arr.reduce((acc, curr) => acc.concat(curr), [])
