...
export const procTExpComponents = (pt: ProcTExp): TExp[] => {
    const retComps = isTupleTExp(pt.returnTE) ? tupleTExpComponents(pt.returnTE) : [pt.returnTE]
    return [...pt.paramTEs].concat(retComps);
};

const tupleTExpComponents = (te: TupleTExp): TExp[] =>
    isEmptyTupleTExp(te) ? [] : te.TEs;
...
const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => {
    ...
    const paramTS = texps.slice(0, pos)
    const retTS = texps.slice(pos + 1, texps.length);
    return (pos === -1) ? parseTupleTExp(texps) :
        (pos === 0) ? makeFailure(`No param types in proc texp - ${texps}`) :
            (pos === texps.length - 1) ? makeFailure(`No return type in proc texp - ${texps}`) :
                (texps.slice(pos + 1).indexOf('->') > -1) ? makeFailure(`Only one -> allowed in a procexp - ${texps}`) :
                    safe2((args: TExp, returnTE: TExp) =>
                        isNonEmptyTupleTExp(args) ? makeOk(makeProcTExp(args.TEs, returnTE)) : makeOk(makeProcTExp([], returnTE)))
                        (parseTExp(texps.slice(0, pos)),
                            retTS.length === 1 ? parseTExp(retTS[0]) : parseTExp(retTS))
...
const parseTupleTExp = (texps: Sexp[]): Result<TupleTExp> => {
    ...
    return isEmptyTuple(texps) ? makeOk(makeEmptyTupleTExp()) : bind(splitEvenOdds(texps),
        (argTEs: Sexp[]) => bind(mapResult(parseTExp, argTEs), (tes: TExp[]) => isNonTupleTExpArr(tes) ? makeOk(makeNonEmptyTupleTExp(tes)) :
            makeFailure(`TupleTExp cannot contain an element of type TupleTExp: ${JSON.stringify(tes)}`)));
};

const isNonTupleTExpArr = (texps: TExp[]): texps is NonTupleTExp[] =>
    texps.reduce((acc: boolean, curr: TExp) => acc && !isTupleTExp(curr), true);

/*
;; Purpose: Unparse a type expression Texp into its concrete form
*/
export const unparseTExp = (te: TExp): Result<string> => {
    const unparseTupleTExp = (tupe: TupleTExp): Result<string[]> =>
        isEmptyTupleTExp(tupe) ? makeOk(["Empty"]) :
            safe2((te: string, tes: string[]) => makeOk([te].concat(chain(te => ['*', te], tes))))
                (unparseTExp(first(tupe.TEs)), mapResult(unparseTExp, rest(tupe.TEs)));

    const unparseProcTExp = (procte: ProcTExp): Result<string[]> =>
        safe2((paramTE: (string[] | string)[], returnTE: string[] | string) =>
            makeOk(unload2dProcComponent(paramTE)
                .concat(['->', ...unloadProcComponent(returnTE)])))
            (mapResult(unparseTExp, procte.paramTEs), unparseTExp(procte.returnTE));

    const unloadProcComponent = (teStr: string[] | string): string[] =>
        isArray(teStr) ? [...teStr] : [teStr];

    const unload2dProcComponent = (teStrArr: (string[] | string)[]): string[] =>
        teStrArr.length === 0 ? ["Empty"] :
            unloadProcComponent(teStrArr[0]).concat(
                chain((te: string[]) =>
                    ['*', ...unloadProcComponent(te)],
                    map(unloadProcComponent, teStrArr.slice(1, teStrArr.length))));
                    ...
                    isVoidTExp(x) ? makeOk('void') :
                        isEmptyTVar(x) ? makeOk(x.var) :
                            isTVar(x) ? up(tvarContents(x)) :
                                isProcTExp(x) ? unparseProcTExp(x) :
...
// ============================================================
// equivalentTEs: 2 TEs are equivalent up to variable raming.
// For example:
