...
export interface Tuple { tag: 'Tuple'; values: Value[] }
export const makeTuple = (values: Value[]): Tuple => ({ tag: 'Tuple', values: values });
                            ...
                            isSymbolSExp(val) ? val.val :
                                isEmptySExp(val) ? "'()" :
                                    isCompoundSExp(val) ? compoundSExpToString(val) :
                                        isTuple(val) ? map((value: Value) => valueToString(value), val.values).join('\n') :
