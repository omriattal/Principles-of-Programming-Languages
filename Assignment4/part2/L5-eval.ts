...
import { map, repeat, zipWith, zip, KeyValuePair } from "ramda";
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, LetValuesExp, isLetValuesExp, BindingValues } from './L5-ast';
...
import { isClosure, makeClosure, Closure, Value, isTuple, Tuple } from "./L5-value";
...
export const applicativeEval = (exp: CExp, env: Env): Result<Value> =>
    // TODO add explicite treatment for Tuple values
...
//LET-VALUES: direct evaluation rule without syntax expansion
//compute the values, extend the env, eval the body
const evalLetValues = (exp: LetValuesExp, env: Env): Result<Value> => {
    const valTuplesRes = mapResult((v: CExp) => applicativeEval(v, env), map((b: BindingValues) => b.valExp, exp.bindingValuess))
    const varArrs = map((b: BindingValues) => b.vars, exp.bindingValuess);
    return bind(valTuplesRes, (valTuples: Value[]) =>
        validateTuplesToVars(varArrs, valTuples) ?
            evalSequence(exp.body, makeExtEnv(unloadVarArrays(varArrs), unloadvalTuplesArray(valTuples), env)) :
            makeFailure("Invalid let exp bindings"))
}

const validateTuplesToVars = (varArrs: VarDecl[][], valTuples: Value[]): valTuples is Tuple[] => {
    const matchs = zip(varArrs, valTuples);
    return matchs.reduce((acc: boolean, curr: KeyValuePair<VarDecl[], Value>) =>
        acc && isTuple(curr[1]) && curr[0].length === curr[1].values.length, true)
}
const unloadVarArrays = (varArrs: VarDecl[][]): string[] =>
    varArrs.reduce((acc: string[], curr: VarDecl[]) => acc.concat(curr.map((v: VarDecl) => v.var)), [])

const unloadvalTuplesArray = (valTuples: Tuple[]): Value[] =>
    valTuples.reduce((acc: Value[], curr: Tuple) => acc.concat(curr.values), [])
