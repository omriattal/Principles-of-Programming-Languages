...
export interface BindingValues { tag: "BindingValues"; vars: VarDecl[]; valExp: CExp }
export const makeBindingValues = (vars: VarDecl[], valExp: CExp): BindingValues =>
	({ tag: "BindingValues", vars: vars, valExp: valExp })
...
export interface LetValuesExp { tag: "LetValuesExp"; bindingValuess: BindingValues[]; body: CExp[] }
export const makeLetValuesExp = (bindingValuess: BindingValues[], body: CExp[]): LetValuesExp => ({ tag: "LetValuesExp", bindingValuess: bindingValuess, body: body })
...
const isGoodBindingValuess = (bindingValuess: Sexp): bindingValuess is [[Sexp], Sexp][] =>
	isArray(bindingValuess) && allT(isArray, bindingValuess) &&
	allT(isArray, bindingValuess.map((bindingValues: Sexp[]) => bindingValues[0]));
...
export const parseLetValuesExp = (bindingValuess: Sexp, body: Sexp[]): Result<LetValuesExp> =>
	isEmpty(body) ? makeFailure('Body of "let-values" cannot be empty') :
		!isGoodBindingValuess(bindingValuess) ? makeFailure(`Invalid BindingValuess: ${JSON.stringify(bindingValuess)}`) :
			safe2((bdgvss: BindingValues[], body: CExp[]) => makeOk(makeLetValuesExp(bdgvss, body)))
				(parseBindingValuess(bindingValuess), mapResult(parseL5CExp, body));

export const parseBindingValuess = (bindingValuess: [[Sexp], Sexp][]): Result<BindingValues[]> =>
	safe2((vds: VarDecl[][], vals: CExp[]) => makeOk(zipWith(makeBindingValues, vds, vals)))
		(mapResult((bdgvs: [[Sexp], Sexp]) => mapResult(parseVarDecl, bdgvs[0]), bindingValuess),
			mapResult(parseL5CExp, map(b => b[1], bindingValuess)));
...
const unparseLetValueExp = (lve: LetValuesExp): Result<string> =>
	safe2((bdgvss: string, body: string) => makeOk(`(let-values (${bdgvss}) ${body})`))
		(unparseBindingValuess(lve.bindingValuess), unparseLExps(lve.body));

const unparseBindingValuess = (bindingValuess: BindingValues[]): Result<string> =>
	bind(mapResult(bdgvs => safe2((vds: string, val: string) => makeOk(`(${vds} ${val})`))
		(unparseVarDeclArray(bdgvs.vars), unparse(bdgvs.valExp)), bindingValuess),
		(bdgvss: string[]) => makeOk(join(" ", bdgvss)));

const unparseVarDeclArray = (vdsArr: VarDecl[]): Result<string> =>
	bind(mapResult((v: VarDecl) => unparseVarDecl(v), vdsArr), (vds: string[]) => makeOk(`(${join(" ", vds)})`))