...
// * <extended-env> ::= (env (symbol+) (value+) next-env) // env(vars:List(Symbol), vals:List(CExp), next-env: Env)
...
export const makeExtEnv = (vs: string[], vals: [CExp,Env][], env: Env): ExtEnv =>
...
// Apply-env
export const applyEnv = (env: Env, v: string): Result<KeyValuePair<CExp, Env>> =>
    isEmptyEnv(env) ? makeFailure(`var not found ${v}`) :
    isExtEnv(env) ? applyExtEnv(env, v) :
    applyRecEnv(env, v);

const applyExtEnv = (env: ExtEnv, v: string): Result<KeyValuePair<CExp, Env>> =>
    env.vars.includes(v) ? makeOk(env.vals[env.vars.indexOf(v)]) :
    applyEnv(env.nextEnv, v);

const applyRecEnv = (env: RecEnv, v: string): Result<KeyValuePair<CExp, Env>> =>
