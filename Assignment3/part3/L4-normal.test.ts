...
import { parseL4, makePrimOp, makeAppExp, makeNumExp, makeIfExp, makeBoolExp } from '../part3/L4-ast';
import { listPrim } from "../part3/evalPrimitive";
import { evalNormalParse, evalNormalProgram } from '../part3/L4-normal';
import { isClosure, Value, makePromise } from '../part3/L4-value';
import { makeOk, isOk, Result, bind } from "../shared/result";
import { makeEmptyEnv } from '../part3/L4-env-normal';
...
describe('Part 3 Tests', () => {
    it('Makes Promise while evaluating VarRef of AppExp', () => {
        const promise = makePromise(makeAppExp(makePrimOp('+'), [makeNumExp(1), makeNumExp(2)]), makeEmptyEnv());
        expect(bind(parseL4('(L4 (define x (+ 1 2)) x)'), evalNormalProgram)).to.eql(makeOk(promise));
    });
    it('Makes Promise while evaluating VarRef of IfExp', () => {
        const promise = makePromise(makeIfExp(makeBoolExp(true), makeNumExp(1), makeNumExp(2)), makeEmptyEnv());
        expect(bind(parseL4('(L4 (define x (if #t 1 2)) x)'), evalNormalProgram)).to.eql(makeOk(promise));
    });
    it('Evaluates a program which will cause an error in applicative evaluation, but not in normal evaluation ', () => {

        const promise = makePromise(makeAppExp(makePrimOp('-'), []), makeEmptyEnv());
        expect(bind(parseL4('(L4 (define x (-)) x)'), evalNormalProgram)).to.eql(makeOk(promise));
    });
